...
// ══════════════════════════════════════════════════════════════════════
// 🎫  AWAZ CONCERT & TICKET SYSTEM
// ══════════════════════════════════════════════════════════════════════

// ── Types ─────────────────────────────────────────────────────────────
interface TicketType {
  id: string;
  name: string;       // "Standard" | "VIP" | custom
  price: number;      // EUR
  total: number;      // capacity
  sold: number;       // sold so far
  color: string;      // display accent
}
interface Concert {
  id: string;
  artistId: string;
  ...
  lat: number;
  lng: number;
  date: string;       // ISO date "2025-09-14"
  time: string;       // "20:00"
...
interface Ticket {
  ...
  qrToken: string;    // UUID — used for validation
...
function concertSoldOut(c: Concert): boolean {
  return c.ticketTypes.every(t => t.sold >= t.total);
}

function concertTotalAvail(c: Concert): number {
  return c.ticketTypes.reduce((s,t) => s + Math.max(0, t.total - t.sold), 0);
}

// Minimal SVG QR-like pattern (deterministic from token, 21×21 grid)
function MiniQR({ token, size=120 }: { token:string; size?:number }) {
  const cell = size / 21;
  // Seed a simple hash from token
  let h = 0;
  for (let i = 0; i < token.length; i++) { h = ((h<<5)-h)+token.charCodeAt(i); h|=0; }
  const rand = (i:number) => { let x=(h^(i*2654435761))>>>0; x=((x>>16)^x)*0x45d9f3b>>>0; return (x>>16)^x; };
  const cells: boolean[][] = Array.from({length:21},(_,r)=>
    Array.from({length:21},(_,c2)=>{
      // Finder patterns (corners)
      if((r<7&&c2<7)||(r<7&&c2>13)||(r>13&&c2<7)) return true;
      if((r===1||r===5)&&c2<7) return false;
      if((r===1||r===5)&&c2>13) return false;
      if(r<7&&(c2===1||c2===5)) return false;
      if(r<7&&(c2===15||c2===19)) return false;
      if(r>13&&(c2===1||c2===5)) return false;
      // timing
      if(r===6||c2===6) return (r+c2)%2===0;
      // data
      return (rand(r*21+c2)%3)===0;
    })
  );
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{display:"block"}}>
      <rect width={size} height={size} fill="#fff" rx="4"/>
      {cells.map((row,r)=>row.map((on,c2)=>on
        ?<rect key={`${r}-${c2}`} x={c2*cell} y={r*cell} width={cell} height={cell} fill="#111"/>
        :null
      ))}
      {/* Center logo */}
      <rect x={size/2-9} y={size/2-9} width={18} height={18} fill="#fff" rx="3"/>
      <text x={size/2} y={size/2+5} textAnchor="middle" fontSize="12" fill={C.gold as string} fontWeight="700">آ</text>
    </svg>
  );
}
...
// ── Static map tile (OpenStreetMap) ────────────────────────────────────
function StaticMap({ lat, lng, zoom=14, w=600, h=220 }:
  { lat:number; lng:number; zoom?:number; w?:number; h?:number }) {
  // Calculate tile coords from lat/lng
  const n = Math.pow(2, zoom);
  const xtile = Math.floor((lng + 180) / 360 * n);
  const ytile = Math.floor((1 - Math.log(Math.tan(lat*Math.PI/180) + 1/Math.cos(lat*Math.PI/180))/Math.PI)/2 * n);
  // Use 3×2 grid of tiles around center
  const tileSize = 256;
  const tiles = [];
  for(let dy=-1; dy<=1; dy++) {
    for(let dx=-1; dx<=1; dx++) {
      tiles.push({x:dx,y:dy,tx:xtile+dx,ty:ytile+dy});
    }
  }
  const cx = (w/2) - tileSize/2;
  const cy = (h/2) - tileSize/2;
  return (
    <div style={{width:"100%",height:h,overflow:"hidden",borderRadius:12,position:"relative",background:C.surface as string}}>
      <div style={{position:"absolute",inset:0,overflow:"hidden"}}>
        {tiles.map(({x,y,tx,ty})=>(
          <img key={`${tx}-${ty}`}
            src={`https://tile.openstreetmap.org/${zoom}/${tx}/${ty}.png`}
            alt=""
            style={{position:"absolute",left:cx+x*tileSize,top:cy+y*tileSize,width:tileSize,height:tileSize,display:"block"}}
          />
        ))}
      </div>
      {/* Pin */}
      <div style={{position:"absolute",left:"50%",top:"50%",transform:"translate(-50%,-100%)",zIndex:2,pointerEvents:"none"}}>
        <div style={{width:24,height:24,borderRadius:"50% 50% 50% 0",background:C.ruby as string,border:"3px solid #fff",transform:"rotate(-45deg)",boxShadow:"0 2px 8px rgba(0,0,0,0.3)"}}/>
      </div>
      {/* Attribution */}
      <div style={{position:"absolute",bottom:4,right:6,fontSize:9,color:"#333",background:"rgba(255,255,255,0.8)",padding:"1px 4px",borderRadius:3,zIndex:3}}>
        © OpenStreetMap
      </div>
    </div>
  );
...
// ── Email ticket HTML generator ────────────────────────────────────────
function generateTicketHtml(ticket: Ticket, qrDataUrl: string): string {
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Ticket — ${ticket.concertTitle}</title>
<style>body{font-family:Georgia,serif;background:#f5f0e8;margin:0;padding:24px}
.card{background:#fff;border-radius:16px;max-width:480px;margin:0 auto;overflow:hidden;border:1px solid #e0d8cc}
.header{background:#07060B;padding:24px;text-align:center}
.gold{color:#C8A84A}.body{padding:24px}.row{display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid #f0e8e0}
.label{color:#888;font-size:12px}.val{font-weight:700;font-size:14px}
.qr{text-align:center;padding:24px 0}.token{font-family:monospace;font-size:11px;color:#aaa;text-align:center}</style></head>
<body><div class="card"><div class="header">
<div style="font-size:22px;color:#C8A84A;margin-bottom:4px">آواز</div>
<div style="color:#fff;font-size:18px;font-weight:700">${ticket.concertTitle}</div>
<div style="color:#C8A84A;font-size:14px;margin-top:4px">${ticket.ticketTypeName}</div>
</div><div class="body">
<div class="row"><span class="label">Artist</span><span class="val">${ticket.artistName}</span></div>
<div class="row"><span class="label">Venue</span><span class="val">${ticket.venue}</span></div>
<div class="row"><span class="label">Date</span><span class="val">${ticket.date} at ${ticket.time}</span></div>
<div class="row"><span class="label">Ticket</span><span class="val">${ticket.ticketTypeName}</span></div>
<div class="row"><span class="label">Name</span><span class="val">${ticket.buyerName}</span></div>
<div class="row"><span class="label">Price</span><span class="val">€${ticket.price}</span></div>
<div class="qr"><img src="${qrDataUrl}" width="160" height="160" alt="QR"/></div>
<div class="token">${ticket.qrToken}</div>
<p style="color:#888;font-size:11px;text-align:center;margin-top:16px">Present this QR code at the entrance · Valid for one person only</p>
</div></div></body></html>`;
}

// ── Supabase concert helpers ───────────────────────────────────────────
async function saveConcertToDB(concert: Concert) {
  const sb = await getSupabase(); if(!sb) return;
  const { error } = await sb.from("concerts").upsert([{
    id: concert.id,
    ...
    ticket_types: concert.ticketTypes,
    status: concert.status,
    created_at: concert.createdAt,
  }], {onConflict:"id"});
  if(error) console.warn("Concert save error:", error.message);
}

async function loadConcertsFromDB(artistId: string): Promise<Concert[]> {
  const sb = await getSupabase(); if(!sb) return [];
  const { data, error } = await sb.from("concerts")
    .select("*").eq("artist_id", artistId).order("event_date");
  if(error){ console.warn("Concerts load:", error.message); return []; }
  return (data||[]).map((r:any) => ({
    id: r.id, artistId: r.artist_id, title: r.title, description: r.description||"",
    venue: r.venue, address: r.address, lat: r.lat||0, lng: r.lng||0,
    date: r.event_date, time: r.event_time||"20:00",
    ticketTypes: r.ticket_types||[], status: r.status||"draft",
    createdAt: r.created_at,
  }));
}

async function loadAllPublishedConcerts(): Promise<(Concert & {artistName:string;artistPhoto?:string})[]> {
  const sb = await getSupabase(); if(!sb) return [];
  const { data, error } = await sb.from("concerts")
    .select("*, artists(name,photo,emoji,genre)")
    .eq("status","published").order("event_date");
  if(error){ console.warn("Public concerts load:", error.message); return []; }
  return (data||[]).map((r:any) => ({
    id:r.id, artistId:r.artist_id, title:r.title, description:r.description||"",
    venue:r.venue, address:r.address, lat:r.lat||0, lng:r.lng||0,
    date:r.event_date, time:r.event_time||"20:00",
    ticketTypes:r.ticket_types||[], status:r.status,
    createdAt:r.created_at,
    artistName: r.artists?.name||"", artistPhoto: r.artists?.photo,
    artistEmoji: r.artists?.emoji, artistGenre: r.artists?.genre,
  }));
}

async function saveTicketToDB(ticket: Ticket) {
  const sb = await getSupabase(); if(!sb) return;
  await sb.from("concert_tickets").upsert([{
    id: ticket.id, concert_id: ticket.concertId,
    ticket_type_id: ticket.ticketTypeId,
    ticket_type_name: ticket.ticketTypeName,
    buyer_name: ticket.buyerName, buyer_email: ticket.buyerEmail,
    stripe_payment_id: ticket.stripePaymentId,
    qr_token: ticket.qrToken, status: ticket.status,
    purchased_at: ticket.purchasedAt,
    concert_title: ticket.concertTitle,
    artist_name: ticket.artistName,
    venue: ticket.venue, event_date: ticket.date,
    event_time: ticket.time, price: ticket.price,
  }], {onConflict:"id"});
}

// ── ATOMIC ticket validation ───────────────────────────────────────────
// Single PostgreSQL RPC: UPDATE ... WHERE status='valid' RETURNING *
// Two simultaneous scans → only ONE wins (row-level lock). Zero-race.
async function validateQRToken(
  qrToken: string,
  concertId?: string
): Promise<{ok:boolean;ticket?:any;msg:string;detail?:string}> {
  const sb = await getSupabase();
  if(!sb) return {ok:false, msg:"No connection", detail:"Cannot reach database"};

  // Sanitise — UUIDs are 36 chars, hex+dashes only
  const uuidRe = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
  if(!uuidRe.test(qrToken.trim()))
    return {ok:false, msg:"Invalid QR code", detail:"Malformed token — not issued by Awaz"};

  try {
    // Atomic RPC: UPDATE concert_tickets SET status=used WHERE qr_token=$1 AND status=valid RETURNING *
    const { data, error } = await sb.rpc("scan_ticket", {
      p_token:      qrToken.trim(),
      p_concert_id: concertId || null,
    });

    if(error) {
      const msg = error.message || "";
      if(msg.includes("ALREADY_USED"))   return {ok:false, msg:"Already scanned", detail:msg.split("|")[1]||""};
      if(msg.includes("WRONG_EVENT"))    return {ok:false, msg:"Wrong event",      detail:"Ticket is for a different concert"};
      if(msg.includes("REFUNDED"))       return {ok:false, msg:"Ticket refunded",  detail:"This ticket was refunded"};
      if(msg.includes("NOT_FOUND"))      return {ok:false, msg:"Ticket not found", detail:"QR not in database — possible forgery"};
      return {ok:false, msg:"Database error", detail:error.message};
    }

    // 0 rows = race condition loser — re-fetch to explain why
    if(!data || (Array.isArray(data) && data.length===0)) {
      const {data:ex} = await sb.from("concert_tickets")
        .select("status,buyer_name,scanned_at,concert_id,ticket_type_name")
        .eq("qr_token", qrToken.trim()).single();
      if(!ex) return {ok:false, msg:"Ticket not found", detail:"QR not in database"};
      if(ex.status==="used") return {ok:false, msg:"Already scanned", ticket:ex,
        detail:`Scanned at ${ex.scanned_at ? new Date(ex.scanned_at).toLocaleTimeString() : "unknown time"}`};
      if(ex.status==="refunded") return {ok:false, msg:"Ticket refunded", detail:"Refunded before event"};
      if(concertId && ex.concert_id !== concertId) return {ok:false, msg:"Wrong event", detail:"Ticket for a different concert"};
      return {ok:false, msg:"Cannot validate", detail:"Contact support"};
    }

    const ticket = Array.isArray(data) ? data[0] : data;
    return {ok:true, ticket, msg:"Valid ticket"};
  } catch(e:any) {
    return {ok:false, msg:"Network error", detail: e?.message||"Check internet connection"};
  }
}

// ── Concert payment constants ─────────────────────────────────────────
const AWAZ_FEE_PCT = 0.10;   // 10% platform fee
const ARTIST_PCT   = 0.90;   // 90% goes to artist via Stripe Connect

// ── Create Stripe Payment Intent for ticket purchase ──────────────────
// Calls Edge Function: create-ticket-payment
// Edge Function creates PaymentIntent with application_fee_amount (10%)
// and transfer_destination = artist's Stripe Connect account.
// Artist receives 90% directly — no manual payouts needed.
async function createTicketPaymentIntent(params: {
  concertId:     string;
  concertTitle:  string;
  artistStripeAccount: string;
  lineItems:     {ticketTypeId:string; name:string; qty:number; price:number}[];
  buyerName:     string;
  buyerEmail:    string;
  totalEur:      number;
}): Promise<{clientSecret:string; paymentIntentId:string}> {
  const res = await fetch(`${SUPA_URL}/functions/v1/create-ticket-payment`, {
    method: "POST",
    headers: {
      "Content-Type":  "application/json",
      "Authorization": `Bearer ${SUPA_KEY}`,
    ...
    body: JSON.stringify({
      ...
      artistStripeAccount:params.artistStripeAccount,
      lineItems:          params.lineItems,
      buyerName:          params.buyerName,
      buyerEmail:         params.buyerEmail,
      totalEurCents:      Math.round(params.totalEur * 100),
      feeCents:           Math.round(params.totalEur * AWAZ_FEE_PCT * 100),
    }),
  });
  const data = await res.json();
...
async function loadStripeJs(): Promise<any> {
  if((window as any).Stripe) return (window as any).Stripe(import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY);
  await new Promise<void>((res,rej)=>{
    const s = document.createElement("script");
    s.src = "https://js.stripe.com/v3/";
    s.onload = () => res();
    s.onerror = () => rej(new Error("Could not load Stripe"));
    document.head.appendChild(s);
  });
  return (window as any).Stripe(import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY);
}

// ── Concert Create/Edit form ───────────────────────────────────────────
function ConcertForm({ artist, concert, onSave, onCancel }:
  { artist:any; concert?:Concert; onSave:(c:Concert)=>void; onCancel:()=>void }) {
  const vp = useViewport();
  const [f, setF] = useState({
    ...
    status: concert?.status||"draft" as Concert["status"],
  });
  const [ticketTypes, setTT] = useState<TicketType[]>(
    concert?.ticketTypes || [
      {id:genUUID(),name:"Standard",price:25,total:100,sold:0,color:C.lapis as string},
      {id:genUUID(),name:"VIP",price:60,total:20,sold:0,color:C.gold as string},
    ]
  );
  const [geocoding, setGeocoding] = useState(false);
  const [mapCoords, setMapCoords] = useState({lat:concert?.lat||0,lng:concert?.lng||0});
  const [err, setErr] = useState("");
  const [saving, setSaving] = useState(false);
  ...
  const lookupAddress = async () => {
    ...
    else setErr("Address not found — try a more specific address");
  };

  const addTicketType = () => setTT(tt=>[...tt,{
    id:genUUID(),name:"General",price:20,total:50,sold:0,
    color:[C.ruby,C.emerald,C.lavender,C.saffron][tt.length%4] as string
  }]);

  const updateTT = (id:string, k:keyof TicketType, v:any) =>
    setTT(tt=>tt.map(t=>t.id===id?{...t,[k]:v}:t));

  const removeTT = (id:string) => setTT(tt=>tt.filter(t=>t.id!==id));

  const handleSave = async () => {
    if(!f.title) { setErr("Title required"); return; }
    if(!f.venue) { setErr("Venue required"); return; }
    if(!f.date)  { setErr("Date required"); return; }
    if(ticketTypes.length===0) { setErr("Add at least one ticket type"); return; }
    setSaving(true);
    let lat=mapCoords.lat, lng=mapCoords.lng;
    if(!lat && f.address) {
      const coords = await geocodeAddress(f.address);
      if(coords) { lat=coords.lat; lng=coords.lng; }
    }
    const c: Concert = {
      id: concert?.id||genUUID(),
      artistId: artist.id,
      title: f.title, description: f.description,
      venue: f.venue, address: f.address,
      lat, lng,
      date: f.date, time: f.time,
      ticketTypes: ticketTypes.map(t=>({...t,sold:t.sold||0})),
      status: f.status,
      createdAt: concert?.createdAt||new Date().toISOString(),
    };
    if(HAS_SUPA) await saveConcertToDB(c);
    setSaving(false);
    onSave(c);
  };

  const p = (k:string,v:any) => setF(prev=>({...prev,[k]:v}));

  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T["2xl"],fontWeight:700,color:C.text as string,marginBottom:4}}>
        {concert?"Edit Concert":"New Concert"}
      </div>

      {err&&<div style={{background:C.rubyS as string,border:`1px solid ${C.ruby}44`,borderRadius:10,padding:"10px 14px",color:C.ruby as string,fontSize:T.sm}}>{err}</div>}

      {/* Basic info */}
      <div style={{background:C.card as string,border:`1px solid ${C.border}`,borderRadius:12,padding:vp.isMobile?"16px":"20px",display:"flex",flexDirection:"column",gap:12}}>
        <div style={{fontSize:T.xs,fontWeight:700,color:C.gold as string,letterSpacing:"0.8px",textTransform:"uppercase" as const,marginBottom:2}}>Concert Details</div>
        <Inp label="Concert Title *" value={f.title} onChange={e=>p("title",e.target.value)} placeholder="e.g. Eid al-Adha Special Night"/>
        <Inp label="Short Description" value={f.description} onChange={e=>p("description",e.target.value)} placeholder="Tell guests what to expect…"/>
        <div style={{display:"flex",gap:10,flexWrap:"wrap" as const}}>
          <div style={{flex:"1 1 160px"}}>
            <Inp label="Date *" type="date" value={f.date} onChange={e=>p("date",e.target.value)}/>
          </div>
          <div style={{flex:"1 1 120px"}}>
            <Inp label="Time *" type="time" value={f.time} onChange={e=>p("time",e.target.value)}/>
          </div>
        </div>
      </div>
      ...
      {/* Location */}
      <div style={{background:C.card as string,border:`1px solid ${C.border}`,borderRadius:12,padding:vp.isMobile?"16px":"20px",display:"flex",flexDirection:"column",gap:12}}>
        <div style={{fontSize:T.xs,fontWeight:700,color:C.gold as string,letterSpacing:"0.8px",textTransform:"uppercase" as const,marginBottom:2}}>Location</div>
        <Inp label="Venue Name *" value={f.venue} onChange={e=>p("venue",e.target.value)} placeholder="e.g. Rockefeller Music Hall, Oslo"/>
        <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
          <div style={{flex:1}}>
            <Inp label="Full Address" value={f.address} onChange={e=>p("address",e.target.value)} placeholder="e.g. Torggata 16, Oslo, Norway"/>
          </div>
          <button onClick={lookupAddress} disabled={geocoding||!f.address}
            style={{height:42,padding:"0 14px",background:C.goldS as string,border:`1px solid ${C.gold}44`,borderRadius:10,color:C.gold as string,fontSize:T.xs,fontWeight:700,cursor:"pointer",fontFamily:"inherit",flexShrink:0,whiteSpace:"nowrap" as const}}>
            {geocoding?"…":"📍 Find"}
          </button>
        </div>
        {mapCoords.lat!==0&&(
          <StaticMap lat={mapCoords.lat} lng={mapCoords.lng} h={180}/>
        )}
      </div>

      {/* Ticket types */}
      <div style={{background:C.card as string,border:`1px solid ${C.border}`,borderRadius:12,padding:vp.isMobile?"16px":"20px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <div style={{fontSize:T.xs,fontWeight:700,color:C.gold as string,letterSpacing:"0.8px",textTransform:"uppercase" as const}}>Ticket Types</div>
          <button onClick={addTicketType} style={{background:C.goldS as string,border:`1px solid ${C.gold}44`,borderRadius:8,padding:"6px 12px",color:C.gold as string,fontSize:T.xs,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>+ Add type</button>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {ticketTypes.map(tt=>(
            <div key={tt.id} style={{background:C.surface as string,borderRadius:10,padding:"12px 14px",border:`1px solid ${tt.color}33`,borderLeft:`4px solid ${tt.color}`}}>
              <div style={{display:"flex",gap:8,flexWrap:"wrap" as const,alignItems:"flex-end"}}>
                <div style={{flex:"1 1 120px"}}>
                  <div style={{fontSize:11,color:C.muted as string,fontWeight:700,marginBottom:4,textTransform:"uppercase" as const,letterSpacing:"0.5px"}}>Name</div>
                  <input value={tt.name} onChange={e=>updateTT(tt.id,"name",e.target.value)}
                    style={{width:"100%",background:C.bg as string,border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 10px",color:C.text as string,fontSize:T.sm,fontFamily:"inherit",outline:"none",boxSizing:"border-box" as const}}/>
                </div>
                <div style={{flex:"0 0 90px"}}>
                  <div style={{fontSize:11,color:C.muted as string,fontWeight:700,marginBottom:4,textTransform:"uppercase" as const,letterSpacing:"0.5px"}}>Price (€)</div>
                  <input type="number" min="0" value={tt.price} onChange={e=>updateTT(tt.id,"price",parseInt(e.target.value)||0)}
                    style={{width:"100%",background:C.bg as string,border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 10px",color:C.text as string,fontSize:T.sm,fontFamily:"inherit",outline:"none",boxSizing:"border-box" as const}}/>
                </div>
                <div style={{flex:"0 0 90px"}}>
                  <div style={{fontSize:11,color:C.muted as string,fontWeight:700,marginBottom:4,textTransform:"uppercase" as const,letterSpacing:"0.5px"}}>Capacity</div>
                  <input type="number" min="1" value={tt.total} onChange={e=>updateTT(tt.id,"total",parseInt(e.target.value)||1)}
                    style={{width:"100%",background:C.bg as string,border:`1px solid ${C.border}`,borderRadius:8,padding:"8px 10px",color:C.text as string,fontSize:T.sm,fontFamily:"inherit",outline:"none",boxSizing:"border-box" as const}}/>
                </div>
                <button onClick={()=>removeTT(tt.id)} style={{height:38,width:38,borderRadius:8,background:"none",border:`1px solid ${C.border}`,color:C.muted as string,cursor:"pointer",fontSize:16,flexShrink:0}}>×</button>
              </div>
              <div style={{fontSize:T.xs,color:C.muted as string,marginTop:6}}>
                {tt.total-tt.sold} available · {tt.sold} sold
                {tt.sold>0&&tt.sold/tt.total>0.7&&<span style={{color:C.ruby as string,fontWeight:700,marginLeft:6}}>⚠ {Math.round((1-tt.sold/tt.total)*100)}% left!</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{display:"flex",gap:10,flexWrap:"wrap" as const,alignItems:"center"}}>
        <div style={{display:"flex",gap:8,flex:1}}>
          {(["draft","published"] as const).map(s=>(
            <button key={s} onClick={()=>p("status",s)}
              style={{flex:1,padding:"10px",borderRadius:10,border:`2px solid ${f.status===s?C.gold:C.border}`,background:f.status===s?C.goldS as string:C.card as string,color:f.status===s?C.gold as string:C.muted as string,fontWeight:700,fontSize:T.sm,cursor:"pointer",fontFamily:"inherit",textTransform:"capitalize" as const}}>
              {s==="published"?"🌍 Publish":"📝 Draft"}
            </button>
          ))}
        </div>
        <div style={{display:"flex",gap:8}}>
          <Btn v="ghost" sz="md" onClick={onCancel}>Cancel</Btn>
          <Btn v="gold" sz="md" onClick={handleSave}>{saving?"Saving…":"Save Concert"}</Btn>
        </div>
      </div>
    </div>
  );
...
// ── Artist Concert Manager (the Concerts tab in ArtistPortal) ──────────
function ArtistConcerts({ artist, notify }: { artist:any; notify:(m:string,t?:string)=>void }) {
  const vp = useViewport();
  const [concerts, setConcerts] = useState<Concert[]>([]);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState<Concert|null|"new">(null);

  useEffect(()=>{
    (async()=>{
      setLoading(true);
      if(HAS_SUPA) {
        const data = await loadConcertsFromDB(artist.id);
        setConcerts(data);
      }
      setLoading(false);
    })();
  ...
  const handleSave = (c:Concert) => {
    setConcerts(prev => {
      const idx = prev.findIndex(x=>x.id===c.id);
      return idx>=0 ? prev.map(x=>x.id===c.id?c:x) : [...prev,c];
    });
    setEditing(null);
    notify(c.status==="published"?"Concert published! 🎫":"Concert saved as draft","success");
  };

  const toggleStatus = async (c:Concert) => {
    const updated = {...c, status:(c.status==="published"?"draft":"published") as Concert["status"]};
    if(HAS_SUPA) await saveConcertToDB(updated);
    setConcerts(prev=>prev.map(x=>x.id===c.id?updated:x));
    notify(updated.status==="published"?"Concert is now live! 🌍":"Concert set to draft","success");
  };

  const deleteConcert = async (id:string) => {
    if(!confirm("Delete this concert?")) return;
    if(HAS_SUPA){const sb=await getSupabase();if(sb)await sb.from("concerts").delete().eq("id",id);}
  ...
  if(editing) return (
    <ConcertForm
      artist={artist}
      concert={editing==="new"?undefined:editing}
      onSave={handleSave}
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div>
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T["2xl"],fontWeight:700,color:C.text as string}}>My Concerts</div>
          <div style={{color:C.muted as string,fontSize:T.sm,marginTop:2}}>Create and manage your concert events</div>
        </div>
        <Btn v="gold" sz="md" onClick={()=>setEditing("new")}>+ New Concert</Btn>
      </div>

      {/* ── Earnings dashboard ── */}
      {concerts.length>0&&(()=>{
        const totalSoldTickets = concerts.reduce((s,c)=>s+c.ticketTypes.reduce((a,t)=>a+t.sold,0),0);
        const grossRevenue     = concerts.reduce((s,c)=>s+c.ticketTypes.reduce((a,t)=>a+t.sold*t.price,0),0);
        const artistRevenue    = Math.round(grossRevenue * ARTIST_PCT * 100)/100;
        const awazTotal        = Math.round(grossRevenue * AWAZ_FEE_PCT * 100)/100;
        const liveCount        = concerts.filter(c=>c.status==="published").length;
        const stripeOk         = !!(artist.stripeConnected && artist.stripeAccount);

        return(
          <div style={{marginBottom:24}}>
            {/* Stripe Connect warning if not set up */}
            {!stripeOk&&(
              <div style={{background:`${C.saffron}11`,border:`1px solid ${C.saffron}44`,borderRadius:12,padding:"12px 16px",marginBottom:14,display:"flex",gap:12,alignItems:"center"}}>
                <span style={{fontSize:20,flexShrink:0}}>⚠️</span>
                <div>
                  <div style={{color:C.saffron as string,fontWeight:700,fontSize:T.sm}}>Stripe not connected</div>
                  <div style={{color:C.muted as string,fontSize:T.xs,marginTop:2}}>
                    Connect Stripe in Settings so ticket payments go directly to your bank. Listing concerts is free — you only pay a 10% fee when tickets sell.
                  </div>
                </div>
              </div>
            )}
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:10}}>
              <div style={{background:C.card as string,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px"}}>
                <div style={{fontSize:T.xs,color:C.muted as string,fontWeight:700,textTransform:"uppercase" as const,letterSpacing:"0.5px",marginBottom:6}}>You receive (90%)</div>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T["2xl"],fontWeight:700,color:C.emerald as string}}>€{artistRevenue.toLocaleString("de",{minimumFractionDigits:2,maximumFractionDigits:2})}</div>
                <div style={{fontSize:T.xs,color:C.muted as string,marginTop:4}}>via Stripe → your bank</div>
              </div>
              <div style={{background:C.card as string,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px"}}>
                <div style={{fontSize:T.xs,color:C.muted as string,fontWeight:700,textTransform:"uppercase" as const,letterSpacing:"0.5px",marginBottom:6}}>Gross ticket revenue</div>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T["2xl"],fontWeight:700,color:C.gold as string}}>€{grossRevenue.toLocaleString("de",{minimumFractionDigits:2,maximumFractionDigits:2})}</div>
                <div style={{fontSize:T.xs,color:C.muted as string,marginTop:4}}>{totalSoldTickets} tickets sold</div>
              </div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10}}>
              {[
                {lbl:"Live concerts", val:liveCount, sub:"published", color:C.gold},
                {lbl:"Platform fee", val:`€${awazTotal.toFixed(2)}`, sub:"10% of sales", color:C.lapis},
                {lbl:"Total concerts", val:concerts.length, sub:"all time", color:C.muted},
              ].map(({lbl,val,sub,color})=>(
                <div key={lbl} style={{background:C.card as string,border:`1px solid ${C.border}`,borderRadius:12,padding:"12px 14px",textAlign:"center" as const}}>
                  <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T.xl,fontWeight:700,color:color as string}}>{val}</div>
                  <div style={{fontSize:10,color:C.muted as string,marginTop:3,fontWeight:600}}>{lbl}</div>
                  <div style={{fontSize:9,color:C.faint as string}}>{sub}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {loading?<div style={{textAlign:"center" as const,padding:"40px",color:C.muted as string}}>Loading…</div>
      :concerts.length===0?(
        <div style={{textAlign:"center" as const,padding:"60px 24px",background:C.card as string,borderRadius:16,border:`2px dashed ${C.border}`}}>
          <div style={{fontSize:48,marginBottom:12}}>🎫</div>
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T.xl,fontWeight:700,color:C.text as string,marginBottom:8}}>No concerts yet</div>
          <div style={{color:C.muted as string,fontSize:T.sm,marginBottom:20,maxWidth:320,margin:"0 auto 20px"}}>Create your first concert and sell tickets directly through Awaz</div>
          <Btn v="gold" sz="lg" onClick={()=>setEditing("new")}>+ Create Your First Concert</Btn>
        </div>
      ):(
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {concerts.map(c=>{
            const totalSold = c.ticketTypes.reduce((s,t)=>s+t.sold,0);
            const totalCap  = c.ticketTypes.reduce((s,t)=>s+t.total,0);
            const revenue   = c.ticketTypes.reduce((s,t)=>s+t.sold*t.price,0);
            const pct = totalCap>0 ? Math.round(totalSold/totalCap*100) : 0;
            const isPast = new Date(c.date) < new Date();
            return (
              <div key={c.id} style={{background:C.card as string,border:`1px solid ${c.status==="published"?C.gold+"44":C.border}`,borderRadius:14,overflow:"hidden" as const}}>
                <div style={{height:3,background:c.status==="published"?`linear-gradient(90deg,${C.gold},${C.saffron})`:`${C.border}` as any}}/>
                <div style={{padding:"16px 18px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,flexWrap:"wrap" as const,marginBottom:10}}>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap" as const,marginBottom:3}}>
                        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T.lg,fontWeight:700,color:C.text as string}}>{c.title}</div>
                        <span style={{fontSize:10,fontWeight:700,borderRadius:20,padding:"2px 8px",background:c.status==="published"?C.goldS as string:C.surface as string,color:c.status==="published"?C.gold as string:C.muted as string,border:`1px solid ${c.status==="published"?C.gold+"44":C.border}`,textTransform:"uppercase" as const}}>
                          {isPast?"Past":c.status}
                        </span>
                      </div>
                      <div style={{color:C.muted as string,fontSize:T.xs}}>
                        📍 {c.venue} &nbsp;·&nbsp; 📅 {c.date} {c.time}
                      </div>
                    </div>
                    <div style={{textAlign:"right" as const,flexShrink:0}}>
                      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T.lg,fontWeight:700,color:C.gold as string}}>€{revenue.toLocaleString()}</div>
                      <div style={{fontSize:T.xs,color:C.muted as string}}>{totalSold}/{totalCap} tickets</div>
                    </div>
                  </div>
                  ...
                  {totalCap>0&&(
                    <div style={{marginBottom:12}}>
                      <div style={{height:6,background:C.surface as string,borderRadius:3,overflow:"hidden" as const}}>
                        <div style={{height:"100%",width:`${pct}%`,background:pct>80?C.ruby as string:C.gold as string,borderRadius:3,transition:"width 0.3s"}}/>
                      </div>
                      <div style={{display:"flex",justifyContent:"space-between",marginTop:4,fontSize:11,color:C.muted as string}}>
                        <span>{pct}% sold</span>
                        <span>{totalCap-totalSold} remaining</span>
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{display:"flex",gap:6,flexWrap:"wrap" as const,marginBottom:12}}>
                    {c.ticketTypes.map(tt=>(
                      <span key={tt.id} style={{fontSize:11,padding:"3px 10px",borderRadius:20,background:`${tt.color}18`,color:tt.color,border:`1px solid ${tt.color}33`,fontWeight:700}}>
                        {tt.name} €{tt.price} · {tt.total-tt.sold} left
                      </span>
                    ))}
                  </div>
                  ...
                  <div style={{display:"flex",gap:8,flexWrap:"wrap" as const}}>
                    <Btn v="ghost" sz="sm" onClick={()=>setEditing(c)}>Edit</Btn>
                    <button onClick={()=>toggleStatus(c)}
                      style={{background:c.status==="published"?C.rubyS as string:C.goldS as string,border:`1px solid ${c.status==="published"?C.ruby+"44":C.gold+"44"}`,color:c.status==="published"?C.ruby as string:C.gold as string,borderRadius:8,padding:"7px 14px",fontSize:T.xs,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
                      {c.status==="published"?"Unpublish":"Publish"}
                    </button>
                    <Btn v="ghost" sz="sm" onClick={()=>deleteConcert(c.id)}>Delete</Btn>
                    {c.status==="published"&&(
                      <a href={`${window.location.origin}/scan/${c.id}`} target="_blank"
                        style={{background:C.lapisS as string,border:`1px solid ${C.lapis}44`,color:C.lapis as string,borderRadius:8,padding:"7px 14px",fontSize:T.xs,fontWeight:700,cursor:"pointer",fontFamily:"inherit",textDecoration:"none",display:"inline-flex",alignItems:"center",gap:4}}>
                        📱 Scanner →
                      </a>
                    )}
                  </div>
...
// ── Public Concerts Browse Page ────────────────────────────────────────
function ConcertsPage({ onBack, session }: { onBack:()=>void; session:any }) {
  const vp = useViewport();
  ...
  useEffect(()=>{
    (async()=>{
      setLoading(true);
      if(HAS_SUPA){
        const data = await loadAllPublishedConcerts();
        setConcerts(data);
      }
      setLoading(false);
    })();
  ...
  if(selected) return (
    <ConcertDetailPage concert={selected} onBack={()=>setSelected(null)} session={session}/>
  );

  return (
    <div style={{paddingTop:vp.isMobile?56:62,paddingBottom:vp.isMobile?90:0,minHeight:"100vh",background:C.bg as string}}>
      <div style={{maxWidth:1000,margin:"0 auto",padding:vp.isMobile?"16px":"40px 48px"}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:4}}>
          <button onClick={onBack} style={{background:"none",border:"none",color:C.muted as string,cursor:"pointer",fontSize:T.sm,fontFamily:"inherit",padding:0}}>← Back</button>
        </div>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T["2xl"],fontWeight:700,color:C.text as string,marginBottom:4}}>Upcoming Concerts</div>
        <div style={{color:C.muted as string,fontSize:T.sm,marginBottom:24}}>Live Afghan music events across Europe</div>

        {loading?(
          <div style={{textAlign:"center" as const,padding:"60px",color:C.muted as string}}>Loading concerts…</div>
        ):concerts.length===0?(
          <div style={{textAlign:"center" as const,padding:"80px 24px"}}>
            <div style={{fontSize:56,marginBottom:16}}>🎫</div>
            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T.xl,fontWeight:700,color:C.text as string,marginBottom:8}}>No upcoming concerts</div>
            <div style={{color:C.muted as string,fontSize:T.sm}}>Check back soon — artists are planning events!</div>
          </div>
        ):(
          <div style={{display:"grid",gridTemplateColumns:vp.isMobile?"1fr":vp.isTablet?"repeat(2,1fr)":"repeat(3,1fr)",gap:16}}>
            {concerts.map(c=>{
              const avail = concertTotalAvail(c);
              const minPrice = Math.min(...c.ticketTypes.map((t:TicketType)=>t.price));
              const soldOut = concertSoldOut(c);
              const isPast = new Date(c.date) < new Date();
              return (
                <div key={c.id} onClick={()=>(!soldOut&&!isPast)&&setSelected(c)}
                  style={{background:C.card as string,border:`1px solid ${C.border}`,borderRadius:16,overflow:"hidden" as const,cursor:soldOut||isPast?"default":"pointer",opacity:isPast?0.6:1,transition:"transform 0.15s,box-shadow 0.15s"}}
                  onMouseEnter={e=>{if(!soldOut&&!isPast){(e.currentTarget as HTMLElement).style.transform="translateY(-2px)";(e.currentTarget as HTMLElement).style.boxShadow=`0 8px 24px ${C.gold}22`;}}}
                  onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.transform="";(e.currentTarget as HTMLElement).style.boxShadow="";}}>
                  {/* Date banner */}
                  <div style={{background:`linear-gradient(135deg,${C.ruby}CC,${C.gold}99)`,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div>
                      <div style={{color:"#fff",fontSize:T.xs,fontWeight:700,opacity:0.85,textTransform:"uppercase" as const,letterSpacing:"0.5px"}}>
                        {new Date(c.date).toLocaleDateString("en",{weekday:"short",month:"short",day:"numeric"})}
                      </div>
                      <div style={{color:"#fff",fontSize:T.sm,marginTop:2}}>{c.time}</div>
                    </div>
                    {soldOut?<span style={{background:"rgba(0,0,0,0.4)",color:"#fff",fontSize:10,fontWeight:700,borderRadius:20,padding:"3px 10px"}}>SOLD OUT</span>
                    :avail<20?<span style={{background:"rgba(168,44,56,0.7)",color:"#fff",fontSize:10,fontWeight:700,borderRadius:20,padding:"3px 10px"}}>{avail} left!</span>
                    :null}
                  </div>
                  <div style={{padding:"14px 16px"}}>
                    {/* Artist */}
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                      {c.artistPhoto
                        ?<img src={c.artistPhoto} alt="" style={{width:32,height:32,borderRadius:6,objectFit:"cover" as const}}/>
                        :<div style={{width:32,height:32,borderRadius:6,background:C.goldS as string,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>{c.artistEmoji||"🎵"}</div>
                      }
                      <div>
                        <div style={{fontSize:T.xs,fontWeight:700,color:C.gold as string}}>{c.artistName}</div>
                        <div style={{fontSize:11,color:C.muted as string}}>{c.artistGenre}</div>
                      </div>
                    </div>
                    <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T.md,fontWeight:700,color:C.text as string,marginBottom:4}}>{c.title}</div>
                    <div style={{color:C.muted as string,fontSize:T.xs,marginBottom:10}}>📍 {c.venue}</div>
                    {c.description&&<div style={{color:C.textD as string,fontSize:T.xs,lineHeight:1.5,marginBottom:10}}>{c.description.substring(0,80)}{c.description.length>80?"…":""}</div>}
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div style={{fontSize:T.sm,fontWeight:700,color:C.gold as string}}>From €{minPrice}</div>
                      {!soldOut&&!isPast&&<span style={{fontSize:T.xs,background:C.goldS as string,color:C.gold as string,border:`1px solid ${C.gold}44`,borderRadius:8,padding:"5px 12px",fontWeight:700}}>Buy Tickets →</span>}
                    </div>
                  </div>
                </div>
              );
...
// ── Concert Detail + Ticket Purchase page ─────────────────────────────
function ConcertDetailPage({ concert, onBack, session }:
  { concert:any; onBack:()=>void; session:any }) {
  const vp = useViewport();
  const { show:notify } = useNotif();
  const [quantities, setQuantities] = useState<Record<string,number>>(
    Object.fromEntries(concert.ticketTypes.map((t:TicketType)=>[t.id,0]))
  );
  const [buyerName,  setBuyerName]  = useState(session?.name||"");
  const [buyerEmail, setBuyerEmail] = useState(session?.email||"");
  const [step, setStep] = useState<"browse"|"checkout"|"paying"|"done">("browse");
  const [tickets, setTickets] = useState<Ticket[]>([]);
  const [err, setErr] = useState("");
  const [payErrDetail, setPayErrDetail] = useState("");

  const total    = concert.ticketTypes.reduce((s:number,t:TicketType)=>s+(quantities[t.id]||0)*t.price,0);
  const totalQty = Object.values(quantities).reduce((s:number,n)=>s+(n as number),0);
  const awazFee  = Math.round(total * AWAZ_FEE_PCT * 100) / 100;
  const artistGets = Math.round(total * ARTIST_PCT * 100) / 100;

  // Handle Stripe redirect-back (payment_intent in URL)
  useEffect(()=>{
    const params = new URLSearchParams(window.location.search);
    const piId   = params.get("payment_intent");
    const piStatus = params.get("redirect_status");
    if(!piId || piStatus !== "succeeded") return;
    // Payment confirmed — provision tickets from DB
    (async()=>{
      setStep("paying");
      const sb = await getSupabase();
      if(!sb){ setStep("browse"); return; }
      const { data } = await sb
      ...
      if(data && data.length > 0){
        setTickets(data.map((r:any)=>({
          id:r.id, concertId:r.concert_id, ticketTypeId:r.ticket_type_id,
          ticketTypeName:r.ticket_type_name, buyerName:r.buyer_name,
          buyerEmail:r.buyer_email, stripePaymentId:r.stripe_payment_id,
          qrToken:r.qr_token, status:r.status, purchasedAt:r.purchased_at,
          concertTitle:r.concert_title, artistName:r.artist_name,
          venue:r.venue, date:r.event_date, time:r.event_time, price:r.price,
        })));
        setStep("done");
        // Clean up URL
  ...
  const handlePurchase = async () => {
    if(!buyerName||!buyerEmail){ setErr("Name and email required"); return; }
    if(!buyerEmail.includes("@")){ setErr("Valid email required"); return; }
    setErr(""); setPayErrDetail(""); setStep("paying");

    // ── Path A: Real Stripe (artist has Connect account) ────────────────
    const artistStripe = concert.artistStripeAccount || concert.artist_stripe_account;
    if(HAS_SUPA && artistStripe && import.meta.env.VITE_STRIPE_PUBLISHABLE_KEY) {
      try {
        const lineItems = concert.ticketTypes
          .filter((t:TicketType)=>(quantities[t.id]||0)>0)
          .map((t:TicketType)=>({ ticketTypeId:t.id, name:t.name, qty:quantities[t.id]||0, price:t.price }));

        // 1. Create Payment Intent via Edge Function (10% fee captured server-side)
        const { clientSecret } = await createTicketPaymentIntent({
          concertId:          concert.id,
          concertTitle:       concert.title,
          artistStripeAccount: artistStripe,
          lineItems,
          buyerName,
          buyerEmail,
          totalEur: total,
        ...
        const returnUrl = `${window.location.origin}${window.location.pathname}?payment_intent={PAYMENT_INTENT}&redirect_status=succeeded&concert=${concert.id}`;
        const { error: stripeErr } = await stripe.confirmPayment({
          clientSecret,
          confirmParams: {
        ...
        if(stripeErr) throw new Error(stripeErr.message);
        // After redirect — execution never reaches here
        return;
      } catch(e:any) {
        setErr("Payment failed — " + (e.message||"please try again"));
        setPayErrDetail(e.message||"");
        setStep("checkout");
        return;
      }
    }

    // ── Path B: Demo / no Stripe Connect ────────────────────────────────
    // Artist not yet on Stripe — still create tickets, flag as pending payment
    const newTickets: Ticket[] = [];
    const sb = await getSupabase();
    for(const t of concert.ticketTypes as TicketType[]) {
      const qty = quantities[t.id]||0;
      for(let i=0;i<qty;i++){
        const ticket: Ticket = {
          id: genUUID(), concertId: concert.id, ticketTypeId: t.id,
          ticketTypeName: t.name, buyerName, buyerEmail,
          stripePaymentId: `demo_${genUUID().slice(0,8)}`,
          qrToken: genUUID(), status: "valid",
          purchasedAt: new Date().toISOString(),
          concertTitle: concert.title, artistName: concert.artistName,
          venue: concert.venue, date: concert.date, time: concert.time, price: t.price,
        };
        newTickets.push(ticket);
        if(sb) await saveTicketToDB(ticket);
      }
    }
    if(sb){
      const updatedTypes = concert.ticketTypes.map((t:TicketType)=>({
        ...t, sold: t.sold + (quantities[t.id]||0)
      }));
      await sb.from("concerts").update({ticket_types:updatedTypes}).eq("id",concert.id);
    }
    setTickets(newTickets);
    setStep("done");
    notify("Tickets confirmed! 🎫","success");
  };

  const dateStr = new Date(concert.date).toLocaleDateString("en",{weekday:"long",year:"numeric",month:"long",day:"numeric"});

  return (
    <div style={{paddingTop:vp.isMobile?56:62,paddingBottom:vp.isMobile?90:0,minHeight:"100vh",background:C.bg as string}}>
      <div style={{maxWidth:760,margin:"0 auto",padding:vp.isMobile?"16px":"40px 48px"}}>
        <button onClick={onBack} style={{background:"none",border:"none",color:C.muted as string,cursor:"pointer",fontSize:T.sm,fontFamily:"inherit",padding:0,marginBottom:20}}>← Back to Concerts</button>

        {step==="done"?(
          // ── Confirmation screen ──
          <div style={{textAlign:"center" as const,padding:"40px 24px"}}>
            <div style={{fontSize:64,marginBottom:16}}>🎉</div>
            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T["2xl"],fontWeight:700,color:C.text as string,marginBottom:8}}>You're going to {concert.title}!</div>
            <div style={{color:C.muted as string,fontSize:T.sm,marginBottom:32}}>Your tickets have been sent to {buyerEmail}</div>

            <div style={{display:"flex",flexDirection:"column",gap:12,maxWidth:400,margin:"0 auto"}}>
              {tickets.map((tk,i)=>(
                <div key={tk.id} style={{background:C.card as string,border:`2px solid ${C.gold}44`,borderRadius:16,overflow:"hidden" as const}}>
                  <div style={{height:3,background:`linear-gradient(90deg,${C.ruby},${C.gold})`}}/>
                  <div style={{padding:"20px",display:"flex",gap:16,alignItems:"center"}}>
                    <MiniQR token={tk.qrToken} size={80}/>
                    <div style={{flex:1,textAlign:"left" as const}}>
                      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T.md,fontWeight:700,color:C.text as string,marginBottom:2}}>{tk.ticketTypeName}</div>
                      <div style={{fontSize:T.xs,color:C.muted as string,marginBottom:4}}>#{i+1} · {concert.title}</div>
                      <div style={{fontSize:T.xs,color:C.gold as string,fontWeight:700}}>€{tk.price}</div>
                      <div style={{fontSize:10,color:C.faint as string,marginTop:4,fontFamily:"monospace"}}>
                        {tk.qrToken.slice(0,18)}…
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div style={{marginTop:24,background:C.card as string,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px 20px",maxWidth:400,margin:"24px auto 0",textAlign:"left" as const}}>
              <div style={{fontWeight:700,color:C.text as string,fontSize:T.sm,marginBottom:8}}>Event Details</div>
              {[["📅 Date",dateStr],["🕐 Time",concert.time],["📍 Venue",concert.venue],["🗺 Address",concert.address]].map(([k,v])=>(
                <div key={k} style={{display:"flex",gap:8,padding:"5px 0",borderBottom:`1px solid ${C.border}`,fontSize:T.xs}}>
                  <span style={{color:C.muted as string,minWidth:80}}>{k}</span>
                  <span style={{color:C.text as string}}>{v}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{textAlign:"center" as const,padding:"80px 24px"}}>
            <div style={{width:40,height:40,border:`3px solid ${C.border}`,borderTopColor:C.gold as string,borderRadius:"50%",animation:"spin 0.8s linear infinite",margin:"0 auto 16px"}}/>
            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T.xl,color:C.text as string}}>Processing payment…</div>
            <div style={{color:C.muted as string,fontSize:T.sm,marginTop:8}}>Secured by Stripe</div>
          </div>
        ):(
          <>
            {/* Concert header */}
            <div style={{background:C.card as string,border:`1px solid ${C.border}`,borderRadius:16,overflow:"hidden" as const,marginBottom:20}}>
              <div style={{background:`linear-gradient(135deg,${C.ruby}CC,${C.gold}88)`,padding:"24px 20px"}}>
                <div style={{color:"rgba(255,255,255,0.85)",fontSize:T.xs,fontWeight:700,textTransform:"uppercase" as const,letterSpacing:"1px",marginBottom:4}}>
                  {new Date(concert.date).toLocaleDateString("en",{weekday:"long",year:"numeric",month:"long",day:"numeric"})} · {concert.time}
                </div>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T["2xl"],fontWeight:700,color:"#fff",marginBottom:8}}>{concert.title}</div>
                <div style={{color:"rgba(255,255,255,0.9)",fontSize:T.sm}}>by {concert.artistName}</div>
              </div>
              <div style={{padding:"16px 20px"}}>
                <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap" as const}}>
                  <span style={{color:C.muted as string,fontSize:T.sm}}>📍</span>
                  <span style={{color:C.text as string,fontSize:T.sm,fontWeight:600}}>{concert.venue}</span>
                  {concert.address&&<span style={{color:C.muted as string,fontSize:T.sm}}>· {concert.address}</span>}
                </div>
                {concert.description&&<p style={{color:C.textD as string,fontSize:T.sm,lineHeight:1.6,margin:0,marginBottom:12}}>{concert.description}</p>}
                {concert.lat!==0&&<StaticMap lat={concert.lat} lng={concert.lng} h={200}/>}
              </div>
            </div>

            {/* Ticket selection */}
            <div style={{background:C.card as string,border:`1px solid ${C.border}`,borderRadius:16,padding:"20px",marginBottom:20}}>
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T.xl,fontWeight:700,color:C.text as string,marginBottom:16}}>Select Tickets</div>
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                {concert.ticketTypes.map((t:TicketType)=>{
                  const avail = t.total - t.sold;
                  const qty = quantities[t.id]||0;
                  const soldOut = avail <= 0;
                  return (
                    <div key={t.id} style={{display:"flex",alignItems:"center",gap:12,padding:"14px 16px",background:C.surface as string,borderRadius:12,border:`1px solid ${qty>0?t.color+"55":C.border}`,transition:"border-color 0.2s",opacity:soldOut?0.5:1}}>
                      <div style={{width:4,height:40,borderRadius:2,background:t.color,flexShrink:0}}/>
                      <div style={{flex:1}}>
                        <div style={{fontWeight:700,color:C.text as string,fontSize:T.sm}}>{t.name}</div>
                        <div style={{fontSize:T.xs,color:C.muted as string,marginTop:2}}>
                          {soldOut?"Sold out":`${avail} available`}
                          {avail>0&&avail<20&&!soldOut&&<span style={{color:C.ruby as string,fontWeight:700,marginLeft:4}}>· Only {avail} left!</span>}
                        </div>
                      </div>
                      <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T.lg,fontWeight:700,color:C.gold as string,minWidth:60,textAlign:"right" as const}}>€{t.price}</div>
                      <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
                        <button onClick={()=>setQuantities(q=>({...q,[t.id]:Math.max(0,(q[t.id]||0)-1)}))}
                          disabled={qty===0} style={{width:32,height:32,borderRadius:"50%",border:`1px solid ${C.border}`,background:C.card as string,color:C.text as string,cursor:qty===0?"not-allowed":"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",opacity:qty===0?0.4:1}}>−</button>
                        <span style={{minWidth:20,textAlign:"center" as const,fontWeight:700,color:C.text as string,fontSize:T.sm}}>{qty}</span>
                        <button onClick={()=>setQuantities(q=>({...q,[t.id]:Math.min(avail,(q[t.id]||0)+1)}))}
                          disabled={soldOut||qty>=avail} style={{width:32,height:32,borderRadius:"50%",border:`1px solid ${C.gold}44`,background:C.goldS as string,color:C.gold as string,cursor:(soldOut||qty>=avail)?"not-allowed":"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center",opacity:(soldOut||qty>=avail)?0.4:1}}>+</button>
                      </div>
                    </div>
                  );
                })}
              ...
              {/* Total + fee split */}
              {totalQty>0&&(
                <div style={{marginTop:16,background:C.goldS as string,border:`1px solid ${C.gold}44`,borderRadius:12,overflow:"hidden" as const}}>
                  <div style={{padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div style={{color:C.gold as string,fontWeight:700,fontSize:T.sm}}>{totalQty} ticket{totalQty>1?"s":""}</div>
                    <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T.xl,fontWeight:700,color:C.gold as string}}>€{total}</div>
                  </div>
                  <div style={{borderTop:`1px solid ${C.gold}22`,padding:"10px 16px",display:"flex",gap:16,flexWrap:"wrap" as const}}>
                    <div style={{display:"flex",alignItems:"center",gap:6,fontSize:T.xs,color:C.muted as string}}>
                      <span style={{width:8,height:8,borderRadius:"50%",background:C.emerald as string,display:"inline-block"}}/>
                      Artist gets <strong style={{color:C.text as string}}>€{artistGets.toFixed(2)}</strong> (90%)
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:6,fontSize:T.xs,color:C.muted as string}}>
                      <span style={{width:8,height:8,borderRadius:"50%",background:C.lapis as string,display:"inline-block"}}/>
                      Platform fee <strong style={{color:C.text as string}}>€{awazFee.toFixed(2)}</strong> (10%)
                    </div>
                  </div>
                </div>
              )}
            ...
            {/* Checkout form */}
            {totalQty>0&&step==="checkout"&&(
              <div style={{background:C.card as string,border:`1px solid ${C.border}`,borderRadius:16,padding:"20px",marginBottom:20}}>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:T.xl,fontWeight:700,color:C.text as string,marginBottom:16}}>Your Details</div>
                {err&&(
                  <div style={{background:C.rubyS as string,border:`1px solid ${C.ruby}44`,borderRadius:10,padding:"10px 14px",color:C.ruby as string,fontSize:T.sm,marginBottom:12}}>
                    {err}
                    {payErrDetail&&<div style={{fontSize:11,marginTop:4,opacity:0.7}}>{payErrDetail}</div>}
                  </div>
                )}
                <div style={{display:"flex",flexDirection:"column",gap:10}}>
                  <Inp label="Full Name *" value={buyerName} onChange={e=>setBuyerName(e.target.value)} placeholder="Your full name"/>
                  <Inp label="Email Address *" type="email" value={buyerEmail} onChange={e=>setBuyerEmail(e.target.value)} placeholder="Tickets will be sent here"/>
                </div>
                <div style={{marginTop:16}}>
                  <Btn v="gold" sz="lg" onClick={handlePurchase} xs={{width:"100%"}}>
                    Pay €{total} · {totalQty} ticket{totalQty>1?"s":""} →
                  </Btn>
                  <div style={{textAlign:"center" as const,marginTop:8,fontSize:T.xs,color:C.muted as string}}>🔒 Stripe · SSL encrypted · 90% goes directly to artist</div>
                </div>
              </div>
            )}

            {totalQty>0&&step==="browse"&&(
              <Btn v="gold" sz="lg" onClick={()=>setStep("checkout")} xs={{width:"100%"}}>
...
// ── QR Scanner / Validator page ────────────────────────────────────────
// ── QR Scanner / Validator — bulletproof, atomic, tamper-proof ────────
// Security model:
//   1. QR contains only UUID token (no personal data, no price, no status)
//   2. validateQRToken calls scan_ticket() RPC which is a single atomic
//      UPDATE ... WHERE status='valid' RETURNING * — impossible to race
//   3. Scanner verifies concert_id matches — cross-event reuse blocked
//   4. scan_count column tracks attempted re-uses for fraud audit log
//   5. Manual token entry requires exact UUID format (spoofing detected)
//   6. Offline fallback: shows clear "no connection" — never false-positive
//   7. Each scan result is timestamped and shown with buyer name for staff
function QRScannerPage({ concertId }: { concertId:string }) {
  type ScanStatus = "idle"|"scanning"|"checking"|"valid"|"used"|"invalid"|"wrongevent"|"refunded"|"offline";
  const [status,   setStatus]   = useState<ScanStatus>("idle");
  const [ticket,   setTicket]   = useState<any|null>(null);
  const [detail,   setDetail]   = useState("");
  const [manualToken, setManualToken] = useState("");
  const [concert,  setConcert]  = useState<any|null>(null);
  const [scanLog,  setScanLog]  = useState<{time:string;name:string;type:string;ok:boolean}[]>([]);
  const [showLog,  setShowLog]  = useState(false);
  const [scanning, setScanning] = useState(false);
  const videoRef  = useRef<HTMLVideoElement>(null);
  const streamRef = useRef<MediaStream|null>(null);
  const scanningRef = useRef(false); // prevent double-scan within one camera session
  const lastTokenRef = useRef(""); // debounce: ignore same token scanned twice in <3s

  useEffect(()=>{
    (async()=>{
      if(concertId==="verify") return; // standalone verify page
      const sb = await getSupabase(); if(!sb) return;
      const {data} = await sb.from("concerts").select("*").eq("id",concertId).single();
      if(data) setConcert({...data, ticketTypes: data.ticket_types||[]});
    })();
    return ()=>{ stopCamera(); };
  },[concertId]);
  ...
  const startCamera = async () => {
    setStatus("scanning"); setScanning(true); scanningRef.current = true;
    ...
    try {
      ...
      setStatus("idle"); setScanning(false); scanningRef.current = false;
  ...
  const runScanner = () => {
    if(!("BarcodeDetector" in window)){
      // BarcodeDetector not supported — show message, rely on manual entry
      setDetail("Camera scanner not supported in this browser. Use manual entry.");
      setStatus("idle"); stopCamera(); return;
    }
    ...
    const loop = async () => {
      ...
      try {
        ...
        if(barcodes.length > 0){
          ...
          const token = raw.split("/verify/").pop()?.split("?")[0] || raw;
          // Debounce: ignore same token within 3 seconds
          if(token === lastTokenRef.current){ requestAnimationFrame(loop); return; }
          lastTokenRef.current = token;
          stopCamera();
          await processToken(token);
          return;
        }
      } catch {/* detector frame error — keep looping */}
  ...
  const processToken = async (token: string) => {
    setStatus("checking"); setTicket(null); setDetail("");
    // Check connectivity first
    if(!navigator.onLine){
      setStatus("offline");
      setDetail("No internet — cannot validate ticket. Reconnect and try again.");
      return;
    }
    const result = await validateQRToken(token, concertId==="verify"?undefined:concertId);
    const now = new Date().toLocaleTimeString("no",{hour:"2-digit",minute:"2-digit",second:"2-digit"});
    setTicket(result.ticket);
    setDetail(result.detail||"");
    if(result.ok){
      setStatus("valid");
      setScanLog(l=>[{time:now,name:result.ticket?.buyer_name||"?",type:result.ticket?.ticket_type_name||"?",ok:true},...l.slice(0,49)]);
    } else {
      const s: ScanStatus = result.msg==="Already scanned"?"used"
                           :result.msg==="Wrong event"?"wrongevent"
                           :result.msg==="Ticket refunded"?"refunded"
                           :result.msg==="Network error"?"offline"
                           :"invalid";
      setStatus(s);
      setScanLog(l=>[{time:now,name:result.ticket?.buyer_name||"?",type:result.msg,ok:false},...l.slice(0,49)]);
    }
  };

  const reset = () => {
    setStatus("idle"); setTicket(null); setDetail(""); setManualToken("");
    lastTokenRef.current = "";
  };

  // ── Visual config per status ─────────────────────────────────────────
  type StatusCfg = { color:string; bg:string; icon:string; title:string; sub:string };
  const cfg: Record<string, StatusCfg> = {
    idle:       {color:"#6B7280",bg:"#141220",icon:"📱",title:"Ready to scan",      sub:"Tap below to open camera"},
    scanning:   {color:"#3B82F6",bg:"#0D1A2D",icon:"◉", title:"Scanning…",         sub:"Hold QR code in front of camera"},
    checking:   {color:"#C8A84A",bg:"#1A1400",icon:"⟳", title:"Checking…",         sub:"Contacting database"},
    valid:      {color:"#22C55E",bg:"#0A1F0A",icon:"✓", title:"VALID — LET IN",    sub:""},
    used:       {color:"#F59E0B",bg:"#1F1200",icon:"⚠", title:"ALREADY SCANNED",   sub:""},
    invalid:    {color:"#EF4444",bg:"#1F0808",icon:"✗", title:"INVALID TICKET",    sub:""},
    wrongevent: {color:"#F97316",bg:"#1F0E00",icon:"⊘", title:"WRONG EVENT",       sub:""},
    refunded:   {color:"#8B5CF6",bg:"#120A1F",icon:"↩", title:"TICKET REFUNDED",   sub:""},
    offline:    {color:"#6B7280",bg:"#0F0F0F",icon:"⚡", title:"NO CONNECTION",     sub:""},
  };
  const c = cfg[status] || cfg.idle;
  const isResult = ["valid","used","invalid","wrongevent","refunded","offline"].includes(status);

  return (
    <div style={{minHeight:"100vh",background:"#07060B",display:"flex",flexDirection:"column",alignItems:"center",padding:"24px 16px",fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{width:"100%",maxWidth:420}}>

        {/* ── Header ── */}
        <div style={{textAlign:"center",marginBottom:20}}>
          <div style={{fontFamily:"'Noto Naskh Arabic',serif",fontSize:26,color:"#C8A84A",marginBottom:2}}>آواز</div>
          <div style={{color:"#EDE4CE",fontSize:17,fontWeight:700,fontFamily:"'Cormorant Garamond',serif",marginBottom:2}}>
            {concert?.title || (concertId==="verify"?"Ticket Verifier":"Concert Entrance")}
          </div>
          {concert&&(
            <div style={{color:"#6B7280",fontSize:12}}>
              📍 {concert.venue} &nbsp;·&nbsp; 📅 {concert.event_date} {concert.event_time}
            </div>
          )}
          {/* Scan log toggle */}
          {scanLog.length>0&&(
            <button onClick={()=>setShowLog(v=>!v)}
              style={{marginTop:10,background:"none",border:"1px solid #2C2840",borderRadius:8,padding:"4px 12px",color:"#8A7D68",fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>
              {showLog?"Hide":"Show"} scan log ({scanLog.length})
            </button>
          )}
        </div>
        ...
        {showLog&&scanLog.length>0&&(
          <div style={{background:"#0F0D16",border:"1px solid #201D2E",borderRadius:12,padding:"12px",marginBottom:16,maxHeight:200,overflowY:"auto"}}>
            <div style={{fontSize:11,color:"#6B7280",fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px",marginBottom:8}}>Scan History</div>
            {scanLog.map((entry,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:"1px solid #201D2E"}}>
                <span style={{color:entry.ok?"#22C55E":"#EF4444",fontSize:14,fontWeight:900}}>{entry.ok?"✓":"✗"}</span>
                <span style={{color:"#EDE4CE",fontSize:12,flex:1,fontWeight:entry.ok?600:400}}>{entry.name}</span>
                <span style={{color:"#6B7280",fontSize:11}}>{entry.type}</span>
                <span style={{color:"#4A4238",fontSize:11}}>{entry.time}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {/* ── Status card ── */}
        <div style={{background:c.bg,border:`2px solid ${c.color}`,borderRadius:20,padding:"28px 24px",textAlign:"center",marginBottom:16,transition:"all 0.3s"}}>
          <div style={{
            width:76,height:76,borderRadius:"50%",
            background:`${c.color}20`,border:`3px solid ${c.color}`,
            display:"flex",alignItems:"center",justifyContent:"center",
            fontSize:status==="checking"?28:30,margin:"0 auto 14px",
            color:c.color,
            animation:status==="checking"?"spin 0.8s linear infinite":status==="scanning"?"pulse 1.5s ease-in-out infinite":"none",
          }}>
            {c.icon}
          </div>
          <div style={{color:c.color,fontSize:20,fontWeight:900,letterSpacing:"0.5px",marginBottom:6}}>{c.title}</div>

          {/* Ticket details when valid */}
          {status==="valid"&&ticket&&(
            <div style={{background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:12,padding:"14px",marginTop:10,textAlign:"left"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <div style={{color:"#86EFAC",fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.5px"}}>Ticket holder</div>
                <div style={{color:"#22C55E",fontSize:11,fontWeight:700}}>{ticket.ticket_type_name||ticket.ticketTypeName}</div>
              </div>
              <div style={{color:"#FFFFFF",fontSize:22,fontWeight:700,fontFamily:"'Cormorant Garamond',serif",marginBottom:4}}>
                {ticket.buyer_name||ticket.buyerName}
              </div>
              <div style={{color:"#86EFAC",fontSize:12}}>{ticket.buyer_email||ticket.buyerEmail}</div>
              <div style={{color:"#4ADE80",fontSize:13,fontWeight:700,marginTop:10,textAlign:"center"}}>
                ✅ Admitted · {new Date().toLocaleTimeString("no",{hour:"2-digit",minute:"2-digit"})}
              </div>
            </div>
          )}

          {/* Already scanned details */}
          {status==="used"&&ticket&&(
            <div style={{background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:12,padding:"14px",marginTop:10,textAlign:"left"}}>
              <div style={{color:"#FCD34D",fontSize:13,fontWeight:700,marginBottom:6}}>
                {ticket.buyer_name||ticket.buyerName||"Unknown"}
              </div>
              <div style={{color:"#F59E0B",fontSize:12}}>{ticket.ticket_type_name||ticket.ticketTypeName}</div>
              {detail&&<div style={{color:"#92400E",fontSize:12,marginTop:6,fontStyle:"italic"}}>{detail}</div>}
              <div style={{color:"#F59E0B",fontSize:12,marginTop:8,fontWeight:700}}>⚠ Do not admit this person again</div>
            </div>
          )}

          {/* Wrong event */}
          {status==="wrongevent"&&(
            <div style={{color:"#FDBA74",fontSize:13,marginTop:8,lineHeight:1.5}}>
              This QR code is for a <strong>different concert</strong>.<br/>
              Do not let this person in.
            </div>
          )}

          {/* Generic detail */}
          {detail&&!["valid","used","wrongevent"].includes(status)&&(
            <div style={{color:`${c.color}99`,fontSize:13,marginTop:8,lineHeight:1.5,fontStyle:"italic"}}>{detail}</div>
          )}

          {/* Offline warning */}
          {status==="offline"&&(
            <div style={{color:"#9CA3AF",fontSize:13,marginTop:8,lineHeight:1.6}}>
              Never accept a ticket without internet.<br/>
              <strong style={{color:"#EF4444"}}>Offline = no validation = security risk.</strong>
            </div>
          )}
        </div>
        ...
        {(status==="scanning"||scanning)&&(
          <div style={{position:"relative",borderRadius:16,overflow:"hidden",marginBottom:16,aspectRatio:"1",background:"#000",maxHeight:340}}>
            ...
            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"none"}}>
              <div style={{width:"60%",height:"60%",position:"relative"}}>
                ...
                {/* Scan line */}
                <div style={{position:"absolute",left:0,right:0,height:2,background:"rgba(59,130,246,0.8)",top:"50%",animation:"pulse 1.5s ease-in-out infinite"}}/>
              </div>
            </div>
            {/* Stop button overlay */}
            <button onClick={()=>{stopCamera();setStatus("idle");}}
              style={{position:"absolute",top:10,right:10,background:"rgba(0,0,0,0.6)",border:"1px solid rgba(255,255,255,0.2)",borderRadius:8,color:"#fff",fontSize:12,fontWeight:700,padding:"6px 10px",cursor:"pointer",fontFamily:"inherit",zIndex:2}}>
              ✕ Stop
            </button>
          </div>
        )}
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          {status==="idle"&&(
            <button onClick={startCamera}
              style={{background:`linear-gradient(135deg,#C8A84A,#8B5200)`,color:"#07060B",border:"none",borderRadius:14,padding:"17px",fontSize:16,fontWeight:800,cursor:"pointer",fontFamily:"inherit",letterSpacing:"0.3px"}}>
              📷 Open Camera
            </button>
          )}
          ...
          {isResult&&(
            <button onClick={reset}
              style={{background:"#141220",border:"1px solid #2C2840",color:"#EDE4CE",borderRadius:14,padding:"15px",fontSize:15,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
              ← Scan Next Ticket
            </button>
          )}

          {/* Manual entry — always available as fallback */}
          {(status==="idle"||isResult)&&(
            <div style={{marginTop:4}}>
              <div style={{color:"#4A4238",fontSize:11,textAlign:"center",marginBottom:8,letterSpacing:"0.3px"}}>— or enter ticket code manually —</div>
              <div style={{display:"flex",gap:8}}>
                <input
                  value={manualToken}
                  ...
                  onPaste={e=>{
                    ...
                    const token = pasted.split("/verify/").pop()?.split("?")[0]||pasted;
                    setManualToken(token);
                    setTimeout(()=>processToken(token),100);
                  }}
                  placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
                  style={{flex:1,background:"#0F0D16",border:`1px solid ${manualToken?"#3B82F6":"#201D2E"}`,borderRadius:10,padding:"11px 12px",color:"#EDE4CE",fontSize:12,fontFamily:"monospace",outline:"none",WebkitTapHighlightColor:"transparent"}}
                />
                <button
                  onClick={()=>{ if(manualToken.trim()) processToken(manualToken.trim()); }}
                  disabled={!manualToken.trim()}
                  style={{background:manualToken?"#1A3F7C":"#141220",border:`1px solid ${manualToken?"#1E4E8C":"#201D2E"}`,color:manualToken?"#7EB3F8":"#4A4238",borderRadius:10,padding:"11px 16px",fontSize:13,fontWeight:800,cursor:manualToken?"pointer":"not-allowed",fontFamily:"inherit",flexShrink:0}}>
                  Check
                </button>
              </div>
            </div>
        ...
        {/* ── Security note ── */}
        <div style={{marginTop:20,padding:"10px 14px",background:"#0F0D16",border:"1px solid #201D2E",borderRadius:10,textAlign:"center"}}>
          <div style={{color:"#4A4238",fontSize:11,lineHeight:1.6}}>
            🔒 Each ticket valid once · Verified against live database<br/>
            Never admit without a green screen
          </div>
        </div>
      </div>
    </div>
...
function ArtistPortal({ user, artist, bookings, session, onLogout, onToggleDay, onMsg, onUpdateArtist, theme, onToggleTheme }) {
  ...
  const content=(
    <div style={{padding:vp.isMobile?"16px":"28px 32px",maxWidth:900}}>
      ...
      {tab==="concerts"&&<ArtistConcerts artist={artist} notify={notify}/>}
...
function AppInner() {
  ...
  React.useEffect(()=>{
    ...
    if(viewParam && ["browse","how","pricing","demo","concerts"].includes(viewParam)){
      setView(viewParam);
    }
  },[]);
  ...
  useEffect(()=>{
    const titles={
      ...
      concerts:"Concerts & Tickets · Awaz",
  ...
  // If URL is /scan/CONCERT_ID — show QR scanner page
  const scanMatch = window.location.pathname.match(/^\/scan\/([a-zA-Z0-9-]+)/);
  if(scanMatch) {
    return(
      <NotifContext.Provider value={{show:notify}}>
        <QRScannerPage concertId={scanMatch[1]}/>
      </NotifContext.Provider>
    );
  }

  // If URL is /verify/TOKEN — just render a standalone verification result
  const verifyMatch = window.location.pathname.match(/^\/verify\/([a-zA-Z0-9-]+)/);
  if(verifyMatch) {
    return(
      <NotifContext.Provider value={{show:notify}}>
        <QRScannerPage concertId="verify"/>
      </NotifContext.Provider>
    );
  }
    ...
    <div key={lang} dir={isRTL?'rtl':'ltr'} translate="no" style={{background:C.bg,minHeight:"100vh",width:"100%",maxWidth:"100%",margin:0,padding:0,overflowX:"hidden",fontFamily:isRTL?"'Noto Naskh Arabic','DM Sans',sans-serif":"'DM Sans',sans-serif",color:C.text}}>
      ...
      <header style={{
        ...
        {vp.isDesktop&&(
          <nav style={{display:"flex",gap:2,alignItems:"center"}}>
            {[[t('browseArtists'),"browse"],["Concerts 🎫","concerts"],[t('howItWorks'),"how"],[t('pricing'),"pricing"],["Artist Demo","demo"]].map(([l,v])=>(
      ...
      {view==="home"&&(
        <div style={{paddingTop:vp.isMobile?56:62}}>
          ...
          {/* ── Concerts Promo Banner ── */}
          <section style={{background:C.surface,borderTop:`1px solid ${C.border}`,borderBottom:`1px solid ${C.border}`}}>
            <div style={{maxWidth:1240,margin:"0 auto",padding:vp.isMobile?"24px 16px":"40px 48px",display:"flex",flexDirection:vp.isMobile?"column":"row",alignItems:"center",gap:vp.isMobile?20:40}}>
              <div style={{flex:1}}>
                <div style={{fontSize:T.xs,fontWeight:700,color:C.ruby,letterSpacing:"1px",textTransform:"uppercase" as const,marginBottom:8}}>🎫 New on Awaz</div>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:vp.isMobile?T["2xl"]:T["3xl"],fontWeight:700,color:C.text,lineHeight:1.1,marginBottom:10}}>
                  Live Concerts &<br/>Ticket Sales
                </div>
                <div style={{color:C.muted,fontSize:T.sm,lineHeight:1.7,maxWidth:480,marginBottom:20}}>
                  Buy tickets to upcoming Afghan music events across Europe. Secure payment, instant QR-code delivery — show up and enjoy.
                </div>
                <div style={{display:"flex",gap:10,flexWrap:"wrap" as const}}>
                  <Btn v="ruby" sz="md" onClick={()=>nav("concerts")}>Browse Concerts →</Btn>
                  <Btn v="ghost" sz="md" onClick={()=>setShowApply(true)}>List as Artist</Btn>
                </div>
              </div>
              <div style={{flexShrink:0,display:"flex",gap:10}}>
                {[
                  {icon:"📍",label:"Live map"},
                  {icon:"🎟️",label:"Instant tickets"},
                  {icon:"📱",label:"QR entry"},
                ].map(({icon,label})=>(
                  <div key={label} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"18px 14px",textAlign:"center" as const,minWidth:80}}>
                    <div style={{fontSize:28,marginBottom:6}}>{icon}</div>
                    <div style={{fontSize:T.xs,color:C.muted,fontWeight:600,whiteSpace:"nowrap" as const}}>{label}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* ── CONCERTS ── */}
      {view==="concerts"&&<ConcertsPage onBack={()=>nav("home")} session={session}/>}
          ...
          <div style={{maxWidth:1200,margin:"0 auto",padding:"40px 48px 0"}}>
            <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr",gap:40,marginBottom:32}}>
              ...
              <div>
                ...
                {[["Browse Artists",()=>nav("browse")],["Concerts 🎫",()=>nav("concerts")],["How It Works",()=>nav("how")],["Pricing",()=>nav("pricing")],["Artist Demo",()=>nav("demo")],["Apply as Artist",()=>setShowApply(true)]].map(([l,fn])=>(
      ...
      {vp.isMobile&&["home","browse","how","pricing","concerts"].includes(view)&&(
        <nav style={{position:"fixed",bottom:0,left:0,right:0,zIndex:100,background:`${C.surface}F8`,backdropFilter:"blur(20px)",display:"flex",alignItems:"stretch",paddingBottom:"env(safe-area-inset-bottom,0px)",height:`calc(58px + env(safe-area-inset-bottom,0px))`}}>
          {[ {id:"home",icon:"🏠",label:t('portalHome'),fn:()=>nav("home")}, {id:"browse",icon:"🎵",label:t('browseArtists'),fn:()=>nav("browse")}, {id:"concerts",icon:"🎫",label:"Concerts",fn:()=>nav("concerts")}, {id:"apply",icon:"✨",label:t('applyAsArtist'),fn:()=>setShowApply(true)},
            session
              ? {id:"logout",icon:"👋",label:t('signOut'),fn:()=>logout()}
              : {id:"signin",icon:"🔑",label:t('signIn'),fn:()=>setShowLogin(true)}
          ].map(({id,icon,label,fn})=>{
            const isActive=(id==="home"&&view==="home")||(id==="browse"&&view==="browse")||(id==="concerts"&&view==="concerts");
            return(
